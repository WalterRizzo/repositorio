...
export default function Expenses() {
  ...
  const [dateFilter, setDateFilter] = useState<{from: string, to: string}>({from: '', to: ''}); // Filtro por fecha (rango)
  ...
  const [currencyFilter, setCurrencyFilter] = useState<string>('all'); // Filtro por moneda
      ...
      <div className="max-w-7xl mx-auto px-4 py-8">
        ...
        {activeTab === 'expenses' && (
          ...
          <div className="grid md:grid-cols-4 gap-6 mb-8">
            {/* GASTOS PENDIENTES */}
            <div className="bg-blue-600 rounded-xl p-6 shadow-md hover:shadow-lg border border-blue-700 transition-all duration-200 hover:-translate-y-1">
              ...
              <div className="text-sm font-semibold text-white uppercase tracking-wide">Gastos Pendientes</div>
            </div>

            {/* GASTOS APROBADOS */}
            <div className="bg-green-600 rounded-xl p-6 shadow-md hover:shadow-lg border border-green-700 transition-all duration-200 hover:-translate-y-1">
              <div className="flex items-center justify-between mb-3">
              ...
              <div className="text-sm font-semibold text-white uppercase tracking-wide">Gastos Aprobados</div>
            </div>

            {/* GASTOS RECHAZADOS */}
            <div className="bg-orange-500 rounded-xl p-6 shadow-md hover:shadow-lg border border-orange-600 transition-all duration-200 hover:-translate-y-1">
              <div className="flex items-center justify-between mb-3">
              ...
              <div className="text-sm font-semibold text-white uppercase tracking-wide">Gastos Rechazados</div>
            </div>
          ...
          <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-200 dark:border-gray-700">
            ...
            <div className="mb-4 flex justify-between items-center">
              <div className="text-sm text-gray-600 dark:text-gray-400">
                📊 Mostrando {Math.min((usersPage - 1) * usersPerPage + 1, users.length)} - {Math.min(usersPage * usersPerPage, users.length)} de {users.length} usuarios
              </div>
              <div className="flex space-x-2">
                ...
                <span className="px-3 py-1 bg-blue-100 text-blue-700 rounded text-sm">
                  Página {usersPage} de {Math.max(1, Math.ceil(users.length / usersPerPage))}
                </span>
                <button
                  onClick={() => setUsersPage(Math.min(Math.ceil(users.length / usersPerPage), usersPage + 1))}
        ...
        <div className="flex justify-between items-center mt-4">
          <button
            className="px-3 py-1 bg-violet-700 text-white rounded disabled:opacity-50"
            onClick={() => setUsersPage(Math.max(1, usersPage - 1))}
            disabled={usersPage === 1}
          >Anterior</button>
        ...
        {/* Sección Historial de Movimientos - Solo para admin/supervisor, oculta en tab 'expenses' */}
  {(userProfile?.role === 'admin' || userProfile?.role === 'supervisor') && activeTab !== 'expenses' && activeTab !== 'dba' && (
          <div>
            {/* Filtros de moneda y fecha (rango) debajo de la barra de acciones */}
            <div className="mb-6"></div>

            <div className="flex flex-wrap gap-4 items-center overflow-x-auto mb-6">
              <label className="text-sm font-bold text-violet-300">Filtrar por moneda:</label>
              <select
                value={currencyFilter}
                onChange={e => setCurrencyFilter(e.target.value)}
                className="px-2 py-1 rounded-xl bg-gray-900 text-violet-300 border border-violet-500 font-bold text-xs"
                style={{ minWidth: 70 }}
              >
                <option value="all">Todas</option>
                {[...new Set(balanceMovements.map(m => m.currency))].map(cur => (
                  <option key={cur} value={cur}>{cur}</option>
                ))}
              </select>
              <label className="text-sm font-bold text-violet-300 ml-2">Fecha desde:</label>
              <input
                type="date"
                ...
                style={{ minWidth: 120 }}
              />
              <label className="text-sm font-bold text-violet-300 ml-2">Fecha hasta:</label>
              <input
                type="date"
                value={dateFilter?.to || ''}
            ...
            <div className="bg-gradient-to-br from-gray-900 via-gray-800 to-gray-900 rounded-2xl shadow-2xl border border-violet-500/20 p-6">
              <div className="mb-6 flex items-center justify-between">
                <div className="flex items-center space-x-2">
                  ...
                  <select
                    value={userFilter}
                    onChange={(e) => { setUserFilter(e.target.value); setMovementsPage(1); }}
                  ...
                  <select
                    ...
                    onChange={(e) => { setMovementsFilter(e.target.value); setMovementsPage(1); }}
                    className="px-4 py-2 bg-gray-800 border border-violet-500/20 rounded-xl text-white hover:bg-gray-700 font-semibold transition-all"
                  >
              ...
              {isLoadingMovements ? (
                ...
                <div className="overflow-x-auto">
                  {(() => {
                    const filtered = balanceMovements
                      .filter(m => movementsFilter === 'all' || m.type === movementsFilter)
                      .filter(m => userFilter === 'all' || m.user_id === userFilter)
                      .filter(m => currencyFilter === 'all' || m.currency === currencyFilter)
                      .filter(m => {
                    ...
                    const totalPages = Math.max(1, Math.ceil(total / movementsPerPage));
                    const startIdx = (movementsPage - 1) * movementsPerPage;
                    const pageMovements = filtered.slice(startIdx, startIdx + movementsPerPage);
                    return (
                      <>
                        <div style={{overflowX:'auto'}}>
                          <table className="w-full min-w-max">
                            <thead className="bg-gradient-to-r from-violet-600 to-purple-600 text-xs">
                              <tr>
                                <th className="px-2 py-2 text-left font-black text-white uppercase tracking-wider whitespace-nowrap" style={{minWidth:70}}>📅 FECHA</th>
                                <th className="px-2 py-2 text-left font-black text-white uppercase tracking-wider whitespace-nowrap" style={{minWidth:70}}>👤 USUARIO</th>
                                <th className="px-2 py-2 text-left font-black text-white uppercase tracking-wider whitespace-nowrap" style={{minWidth:60}}>🎯 TIPO</th>
                                <th className="px-2 py-2 text-left font-black text-white uppercase tracking-wider whitespace-nowrap" style={{minWidth:60}}>💰 MONTO</th>
                                <th className="px-2 py-2 text-left font-black text-white uppercase tracking-wider whitespace-nowrap" style={{minWidth:50}}>🌎</th>
                                <th className="px-2 py-2 text-left font-black text-white uppercase tracking-wider whitespace-nowrap" style={{minWidth:70}}>📊 ANTERIOR</th>
                                <th className="px-2 py-2 text-left font-black text-white uppercase tracking-wider whitespace-nowrap" style={{minWidth:70}}>📈 NUEVO</th>
                                <th className="px-2 py-2 text-left font-black text-white uppercase tracking-wider whitespace-nowrap" style={{minWidth:90}}>📝 DESCRIPCIÓN</th>
                              </tr>
                            </thead>
                            <tbody className="divide-y divide-gray-700">
                              {pageMovements.map((movement) => (
                                <tr key={movement.id} className="hover:bg-violet-900/30 transition-all">
                                  ...
                                  <td className="px-4 py-3 whitespace-nowrap">
                                    <span className="text-xs font-bold text-violet-300 bg-violet-900/30 px-2 py-1 rounded-xl">
                                      {movement.currency}
                                    </span>
                                  </td>
                        ...
                        {/* Pagination controls always visible below table */}
                        <div className="flex flex-wrap justify-between items-center mt-4 gap-2 overflow-x-auto">
                          <span className="text-sm text-gray-400 font-semibold">
                            Mostrando {startIdx + 1}-{Math.min(startIdx + pageMovements.length, total)} de {total} movimientos
                          </span>
                          <div className="flex gap-2">
                            <button
                              disabled={movementsPage === 1}
                              onClick={() => setMovementsPage(movementsPage - 1)}
                              className={`px-3 py-1 rounded-lg font-bold text-xs ${movementsPage === 1 ? 'bg-gray-700 text-gray-400 cursor-not-allowed' : 'bg-violet-600 text-white hover:bg-violet-700'}`}
                            >Anterior</button>
                            {Array.from({ length: totalPages }, (_, i) => (
                              <button
                                key={i + 1}
                                onClick={() => setMovementsPage(i + 1)}
                                className={`px-3 py-1 rounded-lg font-bold text-xs ${movementsPage === i + 1 ? 'bg-purple-600 text-white' : 'bg-gray-800 text-gray-300 hover:bg-violet-700'}`}
                              >{i + 1}</button>
                            ))}
                            <button
                              disabled={movementsPage === totalPages}
                              onClick={() => setMovementsPage(movementsPage + 1)}
                              className={`px-3 py-1 rounded-lg font-bold text-xs ${movementsPage === totalPages ? 'bg-gray-700 text-gray-400 cursor-not-allowed' : 'bg-violet-600 text-white hover:bg-violet-700'}`}
                            >Siguiente</button>
                          </div>
                        </div>
                        {total === 0 && (
